    ...
    if (isMp4Url(embedUrl)) {
        ...
        if (video.src !== embedUrl) {
            ...
            function showDownloadFallback() {
                wrapper.innerHTML = `
                    <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;background:#1a1a1a;padding:20px;text-align:center;border-radius:8px;">
                        <p style="color:#e50914;font-size:1.3rem;margin-bottom:10px;">⚠️ Видео не удалось воспроизвести</p>
                        <p style="color:#aaa;font-size:0.95rem;max-width:400px;">Ваше устройство не поддерживает этот формат видео.</p>
                        <a href="${embedUrl}" download style="display:inline-block;margin-top:20px;padding:14px 40px;background:#4caf50;color:#fff;border-radius:6px;text-decoration:none;font-weight:bold;font-size:1.1rem;">⬇ Скачать файл</a>
                        <p style="color:#666;font-size:0.8rem;margin-top:15px;">После скачивания откройте файл через VLC или MX Player</p>
                    </div>
                `;
            }
            ...
            video.onerror = function() {
                ...
                if (video.error.code === 3) {
                    // Ошибка декодирования — сразу показываем кнопку скачать
                    showDownloadFallback();
                } else if (video.error.code === 2) {
                    // Ошибка сети — пробуем перезапустить
                    attemptRestart();
                } else {
                    showDownloadFallback();
                }
...
document.addEventListener('keydown', function(e) {
    ...
    switch(e.keyCode) {
        ...
        case 39: // Вправо — +5 сек
            ...
            break;
        case 38: // Вверх — громкость + (системная, но перехватываем)
            e.preventDefault();
            break;
        case 40: // Вниз — громкость -
            e.preventDefault();
            break;
        case 70: // F — полноэкранный (если есть клавиатура)
            e.preventDefault();
