...
function loadVideo() {
    ...
    if (isMp4Url(embedUrl)) {
        ...
        if (video.src !== embedUrl) {
            ...
            // Обработчик ошибок (важно для ТВ!)
            video.onerror = function() {
                ...
                else if (video.error.code === 3) msg += 'Ошибка декодирования.';
            ...
            // После загрузки метаданных – пробуем запустить
            video.onloadedmetadata = function() {
                const playPromise = video.play();
                if (playPromise !== undefined) {
                    playPromise.catch(e => {
                        console.log('Автовоспроизведение заблокировано:', e);
                        // На ТВ часто блокируется, пользователь нажмёт кнопку Play
                    });
                }
            };
        } else {
