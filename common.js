...
function loadVideo() {
    ...
    // Если видео-элемента нет – создаём с правильными атрибутами для ТВ
    if (!video) {
        ...
        video.preload = 'metadata';          // Ключевой параметр для ТВ!
    ...
    if (isMp4Url(embedUrl)) {
        ...
        // Если src изменился – перезагружаем
        if (video.src !== embedUrl) {
            ...
            // Функция для сброса и перезапуска
            function attemptRestart() {
                if (retryCount < MAX_RETRIES) {
                    ...
                    video.currentTime = 0;
                    video.load();
                    // Пытаемся запустить через небольшую задержку
                    setTimeout(() => {
                        video.play().catch(e => console.log('Автостарт не удался после перезагрузки', e));
                    }, 500);
                } else {
                    // Если все попытки исчерпаны – показываем сообщение
                    wrapper.innerHTML = `
                        <div style="color: #e50914; text-align: center; padding: 40px; background: #1a1a1a; border-radius: 8px;">
                            <p style="font-size: 1.2rem;">❌ Не удалось воспроизвести видео на вашем устройстве.</p>
                            <p style="font-size: 0.9rem; color: #aaa;">Ошибка декодирования. Попробуйте скачать файл или обновить прошивку телевизора.</p>
                            <button onclick="location.reload()" style="margin-top: 10px; padding: 10px 20px; background: #e50914; color: white; border: none; border-radius: 4px; cursor: pointer;">↻ Обновить страницу</button>
                        </div>
                    `;
                }
            }
            ...
            video.onerror = function() {
                ...
                if (video.error.code === 4) msg += 'Формат не поддерживается.';
                else if (video.error.code === 3) {
                    msg += 'Ошибка декодирования.';
                    // Пробуем перезапустить
                    attemptRestart();
                    return;
                } else if (video.error.code === 2) msg += 'Ошибка сети.';
                else if (video.error.code === 1) msg += 'Загрузка прервана.';
                // Если ошибка не MEDIA_ERR_DECODE (3) – показываем сообщение без перезапуска
                if (video.error.code !== 3) {
                    wrapper.innerHTML = `<div style="color:red;text-align:center;padding:20px;">${msg}</div>`;
                }
            };

            // ---- ОБРАБОТЧИК ЗАВИСАНИЯ (stalled / waiting) ----
            video.onstalled = function() {
                console.log('Видео зависло, пробуем перезапустить');
                attemptRestart();
            };
            video.onwaiting = function() {
                console.log('Видео ожидает буферизации, пробуем перезапустить');
                attemptRestart();
            };

            // ---- ПОСЛЕ ЗАГРУЗКИ МЕТАДАННЫХ ----
            video.onloadedmetadata = function() {
                // Проверяем, поддерживается ли кодек
                const canPlay = video.canPlayType('video/mp4; codecs="avc1.42E01E"') || 
                                video.canPlayType('video/mp4; codecs="avc1.4D401E"') ||
                                video.canPlayType('video/mp4');
                if (canPlay === '') {
                    // Кодек не поддерживается – пробуем другой формат или показываем ошибку
                    wrapper.innerHTML = `
                        <div style="color: #e50914; text-align: center; padding: 40px; background: #1a1a1a; border-radius: 8px;">
                            <p style="font-size: 1.2rem;">⚠️ Видео в формате H.264, но ваше устройство не поддерживает его.</p>
                            <p style="font-size: 0.9rem; color: #aaa;">Попробуйте скачать файл и воспроизвести через внешний плеер.</p>
                            ${video.src ? `<a href="${video.src}" download style="display:inline-block;margin-top:10px;padding:10px 20px;background:#4caf50;color:#fff;border-radius:4px;text-decoration:none;">⬇ Скачать видео</a>` : ''}
                        </div>
                    `;
                    return;
                }
                // Кодек поддерживается – запускаем
                const playPromise = video.play();
                if (playPromise !== undefined) {
                    playPromise.catch(e => {
                        console.log('Автовоспроизведение заблокировано:', e);
                        // Показываем сообщение, чтобы пользователь нажал Play
                    });
                }
            };

            // Если загрузка метаданных не происходит в течение 10 секунд – считаем, что что-то не так
            let metadataTimeout = setTimeout(() => {
                if (!video.duration || video.duration === 0) {
                    console.log('Не удалось получить метаданные, перезапускаем');
                    attemptRestart();
                }
            }, 10000);
            // Очищаем таймаут, если метаданные загрузились
            video.onloadedmetadata = (function(original) {
                return function() {
            ...
            // Если src тот же, но видео остановилось – пробуем перезапустить
            if (video.paused) {
                video.play().catch(() => {});
            }
        }

        // Обновляем URL (сезон/серия)
...
// ============= НАВИГАЦИЯ ПО СЕЗОНАМ (С СОРТИРОВКОЙ ПО ЧИСЛАМ И СОХРАНЕНИЕМ URL) =============
    ...
    modalContent.querySelectorAll('.series-select-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (sKey && srKey && currentFilm.seasons[sKey] && currentFilm.seasons[sKey].series[srKey]) {
                ...
                updateFilmInfo();
                loadVideo(); // теперь с исправленной функцией
                renderSeasonSeriesNav();
